...
class classInputFilter {
 ...
 /** 
   * Constructor for classInputFilter class. Only first parameter is required.
   * @access constructor
 ...
 public function process($source) {
  ...
  if (is_array($source)) {
   foreach($source as $key => $value)
    ...
    if (is_string($value)) $source[$key] = $this->remove($this->decode(trim($value)));
   return $source;
  // clean this string
  
  } else if (is_string($source)) {
   // filter source for XSS and other 'bad' code etc.
   return htmlspecialchars((addslashes(htmlspecialchars(htmlentities(trim($this->remove($this->decode($source))))))));
 ...
 protected function decode($source) {
  ...
  if (version_compare(PHP_VERSION, '5.5.0') <= 0) {
     ...
     /*  $source = preg_replace_callback('/&#(\d+);/me', 
      ...
      $source = preg_replace_callback(
          '/&#(\d+);/m',
          function ($coincidencias) {
              return utf8_encode(chr($coincidencias[1]));
          },
          $source
      );
 ...
 function validarInput($input,$tipo,$validacion = "none") {
 	if (FALSE == isset($input)) return NULL;
 	if (TRUE == is_null($input)) return NULL;
 	if (TRUE == empty($input)) return NULL;
 	switch ($tipo) {
 		case "I" || "i":
 			if (FALSE == is_numeric($input)) return NULL;
 			else return TRUE;
 			break;
 		case "S" || "s":
 			break;
 		case "F" || "f":
 			if (FALSE == is_float($input)) return NULL;
 			else return TRUE;
 			break;
 	}
 }
