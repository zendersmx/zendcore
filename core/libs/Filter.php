...
class Filter
{
    ...
    /**
     * Constructor for classInputFilter class.
    ...
    public function process($source)
    {
        ...
        if (is_array($source)) {
            foreach ($source as $key => $value)
                ...
                if (is_string($value))
                    $source[$key] = $this->remove($this->decode(trim($value)));
            return $source;
            // clean this string
        } else 
            if (is_string($source)) {
    ...
    protected function decode($source)
    {
        ...
        if (version_compare(PHP_VERSION, '5.5.0') <= 0) {
             ...
             * }, $source );
             */
            $source = preg_replace_callback('/&#(\d+);/m', function ($coincidencias)
            {
    ...
    /**
     * Method to be called by another php script.
     * Processes for SQL injection
     * 
     * @access public
     * @param Mixed $source
     *            - input string/array-of-string to be 'cleaned'
     * @param Buffer $connection
...
function filter() {
    static $Filter;
    return isset($Filter) ? $Filter : $Filter = new Filter();
}
?>